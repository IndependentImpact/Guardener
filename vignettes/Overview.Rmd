...
```{r}
dfBlocks %>% filter(blockType == "requestVcDocumentBlock")
```
...
Get the policies and blocks using a particular schema

```{r}
schemaId <- dfSchemas[56, "uuid"] %>% pull()
blockInfo <- whichBlocksHasSchema(dfBlocks = dfBlocks, schemaId = schemaId)
blockID <- blockInfo$id
blockPolicy <- blockInfo$policyId
```
